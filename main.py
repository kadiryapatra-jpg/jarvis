...
def _get_api_key() -> str:
    ...
    try:
        ...
        raise RuntimeError(
            "Gemini API key is missing. Enter it in the setup dialog or set GEMINI_API_KEY."
        ) from exc
    except (json.JSONDecodeError, OSError, AttributeError) as exc:
        raise RuntimeError(f"Could not read {API_CONFIG_PATH}: {exc}") from exc

    if not key:
        raise RuntimeError(
            "Gemini API key is empty. Enter it in the setup dialog or set GEMINI_API_KEY."
        )
    return key
...
class JarvisLive:
    ...
    def _on_text_command(self, text: str):
        ...
        if not self._loop or not self.session:
            ...
            self.ui.write_log("SYS: Still connecting; command queued.")
            return
    ...
    def _send_text(self, text: str) -> None:
        try:
            ...
            self.ui.write_log(f"ERR: Could not send command: {exc}")
            return
        ...
        def report_failure(done):
            try:
                ...
                self.ui.write_log(f"ERR: Gemini command failed: {exc}")
